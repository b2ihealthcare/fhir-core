 ...
 * FHIR R5 {@code ConceptMap} resources via a Jackson {@link ObjectMapper}.
 *
 * @since 0.5.0
 */
public class FhirR5JsonModuleConceptMapTest {
	...
	private static ConceptMap buildConceptMap() {
		...
		cm.setId("r5-cm");
		cm.setUrl("http://example.com/r5/cm");
		cm.setVersion("1.0.0");
		cm.setName("R5ExampleCM");
		cm.setTitle("R5 Example Concept Map");
		...
		cm.addGroup()
			...
			.addElement()
				...
				.addTarget()
					.setCode("target-code")
					.setRelationship(Enumerations.ConceptMapRelationship.EQUIVALENT);
		return cm;
	}

	private static void assertConceptMap(final ConceptMap cm) {
		assertThat(cm.getId()).isEqualTo("r5-cm");
		assertThat(cm.getUrl()).isEqualTo("http://example.com/r5/cm");
		assertThat(cm.getVersion()).isEqualTo("1.0.0");
		assertThat(cm.getName()).isEqualTo("R5ExampleCM");
		assertThat(cm.getTitle()).isEqualTo("R5 Example Concept Map");
		...
		assertThat(cm.getGroupFirstRep().getElementFirstRep().getTargetFirstRep().getRelationship()).isEqualTo(Enumerations.ConceptMapRelationship.EQUIVALENT);
	}
	...
	public void serialize_conceptMap() throws Exception {
		...
		assertThat(json).contains("\"url\":\"http://example.com/r5/cm\"");
		assertThat(json).contains("\"version\":\"1.0.0\"");
		assertThat(json).contains("\"name\":\"R5ExampleCM\"");
		...
		assertThat(json).contains("\"relationship\":\"equivalent\"");
	}
	...
	public void deserialize_from_json_literal() throws Exception {
		final String json = """
		{
		  ...
		  "id": "r5-cm",
		  "url": "http://example.com/r5/cm",
		  "version": "1.0.0",
		  "name": "R5ExampleCM",
		  "title": "R5 Example Concept Map",
		  "status": "active",
		  "group": [
		    {
