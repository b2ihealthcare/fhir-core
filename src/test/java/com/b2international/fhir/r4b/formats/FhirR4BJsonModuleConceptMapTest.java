 ...
 * FHIR R4B {@code ConceptMap} resources via a Jackson {@link ObjectMapper}.
 *
 * @since 0.5.0
 */
public class FhirR4BJsonModuleConceptMapTest {
	...
	private static ConceptMap buildConceptMap() {
		...
		cm.setId("r4b-cm");
		cm.setUrl("http://example.com/r4b/cm");
		cm.setVersion("1.0.0");
		cm.setName("R4BExampleCM");
		cm.setTitle("R4B Example Concept Map");
		...
		cm.addGroup()
			...
			.addElement()
				...
				.addTarget()
					.setCode("target-code")
					.setEquivalence(ConceptMap.ConceptMapEquivalence.EQUAL);
		return cm;
	}

	private static void assertConceptMap(final ConceptMap cm) {
		assertThat(cm.getId()).isEqualTo("r4b-cm");
		assertThat(cm.getUrl()).isEqualTo("http://example.com/r4b/cm");
		assertThat(cm.getVersion()).isEqualTo("1.0.0");
		assertThat(cm.getName()).isEqualTo("R4BExampleCM");
		assertThat(cm.getTitle()).isEqualTo("R4B Example Concept Map");
		...
		assertThat(cm.getGroupFirstRep().getElementFirstRep().getTargetFirstRep().getEquivalence()).isEqualTo(ConceptMap.ConceptMapEquivalence.EQUAL);
	}
	...
	public void serialize_conceptMap() throws Exception {
		...
		assertThat(json).contains("\"url\":\"http://example.com/r4b/cm\"");
		assertThat(json).contains("\"version\":\"1.0.0\"");
		assertThat(json).contains("\"name\":\"R4BExampleCM\"");
	...
	public void deserialize_from_json_literal() throws Exception {
		final String json = """
		{
		  ...
		  "id": "r4b-cm",
		  "url": "http://example.com/r4b/cm",
		  "version": "1.0.0",
		  "name": "R4BExampleCM",
		  "title": "R4B Example Concept Map",
		  "status": "active",
		  "group": [
		    {
